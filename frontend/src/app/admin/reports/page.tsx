        ...
        <>
            ...
            <div className="glass rounded-3xl overflow-hidden border-slate-200 shadow-xl">
                <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                    <h3 className="font-black text-slate-400 uppercase tracking-widest text-xs">ประวัติการขายล่าสุด</h3>
                </div>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">
                                ...
                                <th className="px-6 py-4">หมายเลขคำสั่งซื้อ</th>
                                <th className="px-6 py-4">ประเภทการชำระ</th>
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {isLoading ? (
                                <tr><td colSpan={6} className="text-center py-20 animate-pulse text-slate-300 font-bold uppercase tracking-widest">กำลังโหลดข้อมูลการขาย...</td></tr>
                            ) : sales?.map((sale) => (
                                <tr key={sale.id} className="hover:bg-slate-50 transition-colors group">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase ${sale.paymentMethod === 'CASH' ? 'bg-blue-50 text-blue-600' : 'bg-purple-50 text-purple-600'
                                            }`}>
                                            {sale.paymentMethod}
                                        </span>
                                    </td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
                    <div className="glass max-w-md w-full rounded-3xl p-8 shadow-2xl border-white overflow-hidden flex flex-col max-h-[90vh]">
                        <div className="text-center mb-8">
                            <h2 className="text-2xl font-black text-slate-900 leading-none">RECEIPT</h2>
